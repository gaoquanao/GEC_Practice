...
int run_asr(UserData *udata)
{
	...
	// 离线语法识别参数设置 可以将result_type = json 来将返回结果改为json文件
	...
	if (NULL != rec_rslt)
	{
		...
		fprintf(xml_file, rec_rslt);
		fclose(xml_file);

		printf("[AI talk]:原始识别结果:%s\n", rec_rslt);
		xml_read("test.xml", pro); //解析xml数据中的slot并将识别结果插入到pro中
		host2net(pro); //转换为大端字节序
		send_voice_data(pro, 8); //发送pro到ARM开发板
		printf("[AI talk] : sending recognition result to ARM\n");
		free(pro);
	}
	else
	{
		printf("[AI talk] : no recognition result\n");
	}
	printf("=============================================================\n");
	close_voice();
	goto run_exit;
...
void * aitalk(void * arg)
{
	const char *login_config    = "appid = 5992be84"; //登录参数
	UserData asr_data; 
	int ret                     = 0 ;
	char c;
